...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\textbf{回答：}错。TFLOPS 衡量并行吞吐，GHz 表示时钟频率，都不能直接代表单条指令的延迟。
  \item HBM 的“高带宽”指大块连续访问时的吞吐，零散的随机访问达不到标称值。
  \par\textbf{回答：}对。连续访问便于合并访存；随机访问会产生分散的内存事务，难以达到标称带宽。
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。
  \par\textbf{回答：}对。步骤间的依赖限制并行度，总耗时主要由串行依赖链的延迟决定。
  \item “算力 1000 TFLOPS”意味着每次运算的延迟是 $10^{-15}$ 秒。
  \par\textbf{回答：}错。1000 TFLOPS 是整体吞吐指标，其倒数不是单次运算延迟。
\end{enumerate}

\prob{1.2}{CONCEPT}
Session 1 讲座里提过“N 方过百万”这个例子。总计算量 $10^{12}$ FLOP 在当代 GPU 上的运算时间大概是毫秒级，那为什么一个严格在线的串行算法仍然做不到几秒内跑完？（从“延迟”和“吞吐”的角度考虑）

\par\textbf{回答：}GPU 依靠大量并行任务隐藏延迟并发挥吞吐；严格串行时，每一步都要等待上一步，既无法隐藏延迟，也无法利用并行算力。
...
\par\textbf{回答：}SIMD 用一条显式向量指令处理多个元素；SIMT 让程序员编写独立标量线程，硬件再将其组成 warp 共同发射。判断为错：Volta 的独立线程调度没有改变 warp 发射方式，分支路径仍需分别执行，空闲 lane 仍会造成损失。
...
\par\textbf{回答：}
\begin{enumerate}[label=(\alph*)]
  \item CPU 针对单线程低延迟优化；\texttt{<<<1,1>>>} 只有一个有效 lane，既浪费 GPU 资源，也没有其他 warp 隐藏延迟，因此更慢。
  \item 铺满 grid 后，大量 block 分布到多个 SM，活跃 warp 可隐藏延迟并利用带宽，说明 GPU 加速依赖大规模并行吞吐。
\end{enumerate}
...
\par\textbf{回答：} (a)~kernel 异步执行，CPU 读结果前必须等待 GPU 写完。当前版本显式同步；原版本的 device-to-host \texttt{cudaMemcpy} 也会等待此前的 kernel。

(b)~显式内存版为 $110.9\,\mathrm{ms}$，Unified Memory 版为 $94.9\,\mathrm{ms}$，后者快约 $1.17$ 倍。本机的按需页面迁移开销较低，但结果依赖硬件互连和迁移策略。

\prob{2.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item \texttt{vectorAdd<<<...>>>(...)} 这条语句返回时，kernel 一定已经执行完毕。
  \par\textbf{回答：} 错。kernel 启动通常是异步的，返回只表示任务已提交。
  \item 同一个 stream 里，\texttt{cudaMemcpy}（device 到 host）会等它前面的 kernel 全部完成后才开始拷贝。
  \par\textbf{回答：} 对。同一 stream 按顺序执行，同步拷贝会等待此前的 kernel。
  \item kernel 内部的非法访存，会在启动语句处同步地报出来。
  \par\textbf{回答：} 错。启动处只能立即发现配置错误；非法访存等异步错误要到同步或后续 CUDA API 调用时才会报告。
\end{enumerate}
...
\par\textbf{回答：} 每个 block 的 2048 个线程超过 \texttt{maxThreadsPerBlock}，导致 \texttt{invalid configuration argument}。加入启动错误检查并将线程数改为 1024 后通过；该上限针对单个 block。
...
\par\textbf{回答：} Grid-stride loop 用固定线程数处理任意规模数据，并保持合并访存。代价是循环开销；block 太少时还会降低并行度和延迟隐藏能力。
...
\par\textbf{回答：} (a)~顺序由 GPU 根据 SM 和资源状态调度，不保证按 \texttt{blockIdx.x} 排列。
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\textbf{回答：}线性编号为 $3+5\times8=43$；warp 编号为
  $\lfloor43/32\rfloor=1$（第 2 个），lane 编号为 $43\bmod32=11$（第 12 个）。

  \item 这个 block 一共占多少个 warp？

  \par\textbf{回答：}$8\times8=64$ 个线程，恰好占 2 个 warp。

  \item 若 \texttt{blockDim = (33, 1, 1)}，占几个 warp？这样配置浪费在哪里？

  \par\textbf{回答：}占 2 个 warp；第二个 warp 仅 1 个 lane 有效，浪费 31 个执行槽位。
\end{enumerate}
...
\par\textbf{预测：}按 warp 分支约快 2 倍；奇偶分支使每个 warp 串行执行两条路径，而按 warp 分支不发散。
...
\par\textbf{回答：}实测为 $0.387\ \mathrm{ms}$ 和 $0.200\ \mathrm{ms}$，比值 $1.94$，符合预测；偏离 2 倍来自共同开销和测量波动。若两条路径计算量不同，奇偶分支的单 warp 耗时近似为两者之和；按 warp 分支时，每个 warp 只承担所选路径，grid 耗时由两类 warp 的总工作量和较重路径的收尾决定。
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\textbf{回答：}线程读取的是其他线程写入的 \texttt{buf[255-t]}。没有 \texttt{\_\_syncthreads()} 时可能先读后写，产生竞争；该同步保证所有写入完成并可见。

  \item (Optional) 注释掉 sync 后，翻转后的数组错的位置比较随机，但是有些位置一直是对的，试解释原因。（tip: 算一算 $t$ 与 $255-t$ 有没有可能落在同一个 warp）

  \par\textbf{回答：}$t$ 在 warp $w$，$255-t$ 在 warp $7-w$；$w=7-w$ 无整数解，因此二者总在不同 warp。部分位置持续正确只是实际调度顺序较稳定的偶然结果，CUDA 不作保证。
\end{enumerate}

\prob{3.4}{CONCEPT}
\texttt{\_\_syncthreads} 只能同步本 block 内的 threads，那需要全 grid 同步时，标准做法是什么？

\par\textbf{回答：}把计算拆成同一 stream 中的多个 kernel；后一个 kernel 会等待前一个全部完成，因此 kernel 边界就是全 grid 同步，阶段间通过 global memory 传递数据。
...
\par\textbf{结果与回答：}关闭 device 优化后，三版均 PASS：interleaved 为 $0.0369\ \mathrm{ms}$，contiguous 和 shuffle 均为 $0.0246\ \mathrm{ms}$。交错版因取模、活跃 lane 分散和分支发散而慢 $1.50\times$；本次测试未测出 shuffle 的额外加速。
...
\par\textbf{回答：}两版均 PASS，耗时均为 $0.0718\ \mathrm{ms}$，比值 $1.00\times$。constant cache 的优势是同一 warp 读取同一地址时可广播数据；本题仅有 8 个系数，global 版本也容易命中缓存，因此未测出收益。

\prob{4.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item local memory 的“local”指作用域私有，它实际上在片外显存里。
  \par\textbf{回答：}对。local 表示每个线程私有，物理上位于片外显存，并可经过 L1/L2 cache。
  \item 对数组用运行期才知道的下标做索引，可能迫使它被放进 local memory。
  \par\textbf{回答：}对。寄存器不能动态寻址，编译器可能把这类数组放入可寻址的 local memory。
\end{enumerate}
...
\par\textbf{回答：}naive 为 $2.6092\ \mathrm{ms}$，私有化版为 $0.0754\ \mathrm{ms}$，提速 $34.60\times$。私有化把大部分竞争转移到低延迟的 block-local shared memory，最后每个 block 仅合并 256 个 bucket，大幅减少了 global atomic 操作和跨 block 竞争。
...
\par\textbf{回答：}stride 从 1 增至 8 时，warp 的访问逐渐分散，需要更多内存事务，带宽下降。由于 $n$ 和 stride 都是 2 的幂，取模后实际只访问 $n/\text{stride}$ 个不同元素；stride 继续增大时，工作集缩小、缓存复用增强，并受 cache set 与 memory partition 映射影响，因而出现非单调波动。表中数值按逻辑读写量计算，不等同于实际 DRAM 流量。
...
\par\textbf{回答：}(a) 以 29.0 KB 为例，每个 SM 最多驻留 $\lfloor100/29\rfloor=3$ 个 block，occupancy 为 $3\times256/1536=50\%$，与 API 一致。
...
\par\textbf{回答：}(a) cudaEvent 测得的 0.0665 ms 可作为 kernel 耗时。
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\textbf{回答：}正确。同一 stream 内的操作按入队顺序执行。
  \item kernel 启动后，host 代码立刻继续往下执行。
  \par\textbf{回答：}正确。kernel launch 通常对 host 异步，除非随后显式同步。
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。
  \par\textbf{回答：}正确。CPU 缺页后，运行时会协调访问并将该页迁移到 CPU。
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\textbf{回答：}错误。tile 是 block 内的不可变值，且不一定存于显存；写回显存应对 global array 执行 store。
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。
  \par\textbf{回答：}正确。编译器负责把 tile 运算分配给 block 内的硬件线程。
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。
  \par\textbf{回答：}错误。二者可以存在于同一程序中，编程模型按 kernel 选择。
\end{enumerate}
...
编号 & \texttt{blockIdx} / \texttt{threadIdx} & \texttt{ct.bid(axis)} & \texttt{tl.program\_id} \\
数据分工 & 线程用全局下标来划分数据 & block 按编号选择 tile，tile 内由编译器分工 & program 用偏移向量划分数据 \\
边界处理 & \texttt{if} 判断 & load 填充、store 忽略越界元素 & load/store 使用 mask \\
\bottomrule
\end{tabular}
\end{center}
...
\par\textbf{回答：}(a) 由编译器决定；程序员只指定 block 处理的 tile。

(b) 线程块大小、\texttt{blockIdx}/\texttt{threadIdx}/\texttt{blockDim}、每个线程的全局下标以及 \texttt{if} 越界判断均未体现。
...
\par\textbf{回答：}改动集中在 kernel 参数和逐元素计算表达式。新旧操作的数据形状相同，因此数据分块、索引、边界 mask 和 load/store 逻辑都可复用。
...
\par\textbf{回答：}行列号对应 \texttt{T.copy} 中的 tile 起点，grid 尺寸对应 \texttt{T.Kernel} 的两个参数；显式边界判断消失了，越界读取的填充和越界写入的忽略由 \texttt{T.copy} 处理。

\prob{7.5}{CONCEPT}
补全下表，每个空填“用户”或“编译器”（二者都涉及的要写清楚各自的范围）。

\begin{center}\small
\begin{tabular}{@{}lp{6em}p{8em}p{8em}p{9em}@{}}
\toprule
谁负责 & CUDA SIMT & cuTile & Triton & TileLang \\
\midrule
线程到数据的映射 & 用户 & 编译器 & 编译器（用户划分 program） & 编译器（用户定义 tile / 循环） \\
边界处理 & 用户 & 编译器（用户选 padding） & 用户提供 mask & 二者：\texttt{T.copy} 自动，直接访问可显式判断 \\
tile / block 尺寸的选择 & 用户 & 用户定 tile，编译器定线程数 & 用户定 tile / \texttt{num\_warps} & 用户定 tile / threads \\
block 内同步 & 用户 & 编译器 & 编译器（必要时用户可插 barrier） & 二者：高层操作由编译器，低层代码由用户 \\
\bottomrule
\end{tabular}
\end{center}
...
\par\textbf{回答：}Triton 只表达 block tensor 的 load、\texttt{tl.dot} 和数据依赖，编译器据此决定寄存器与 shared memory 的布局，并插入同步和流水调度；用户仍需选择 tile 尺寸。
